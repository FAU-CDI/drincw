...
type Options struct {
	...
	IsolateChildBundles  bool // should each child bundle re-draw their own IDs?
	...
	BundleColor           string // color to highlight starting points for bundles in
}
...
func addField(g *dot.Graph, field pathbuilder.Field, bundle *pathbuilder.Bundle, opts Options, gs map[*dot.Graph]struct{}) {
	...
	for i := 0; i < len(field.PathArray); i += 2 {
		...
		if !reuse && i == maxParentID {
			now.Attr("fontcolor", opts.BundleColor).Attr("color", opts.BundleColor)
		}
...
func (opts Options) NodeID(id string, bundle *pathbuilder.Bundle) string {
	id = opts.IDPrefix + ":::" + id
	if opts.IsolateChildBundles && bundle != nil {
